...
if [ "$#" -ne 5 ]; then
    echo "Usage: $0 <target_protein_file> <feature table> <evalue> <interpro executable path> <threads>"
    echo "./get_custom_reference_bgc_domains.sh target_protein_file.faa target_feature_table.txt 0.05 /path/to/interproscan.sh 20"
    echo "The feature table should have the first column named 'protein' matching the fasta accession headers. The next columns are customizable."
    exit 1
fi
...
# Define the uniprot fasta file and prefix
...
# Load feature table
feature_table = pd.read_csv(f"${FEATURE_TABLE}", sep="\t")

# Check if the first column is named 'protein'
if feature_table.columns[0] != "protein":
    raise ValueError("The first column of the feature table must be named 'protein' to match fasta accessions.")

# Merge filtered_domains and feature table
filtered_domains_features = pd.merge(filtered_domains, feature_table, on="protein", how="left")
...
filtered_interpro_by_protein = filtered_domains_features.groupby(['protein','product'])['interpro_acc'].unique()
filtered_domains_by_protein = filtered_domains_features.groupby(['protein','product'])['signature_acc'].unique()

# Save the significant domain ids and interpro ids
with open(f"${TARGET_PREFIX}_domains_by_protein.txt", "w") as outfile:
    for (protein, product), domain_ids in filtered_domains_by_protein.items():
        interpro_ids = filtered_interpro_by_protein.get((protein, product), [])
        outfile.write(f"{protein}\t{product}\t{','.join(domain_ids)}\t{','.join(interpro_ids)}\n")
EOF